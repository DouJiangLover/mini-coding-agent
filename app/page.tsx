...
const SUGGESTIONS = [
  { icon: '⌁', title: '修复失败测试', prompt: '修复示例项目中失败的测试，只做必要修改，并确保所有测试通过。' },
  { icon: 'T', title: '补充单元测试', prompt: '分析当前项目，为核心功能补充必要的单元测试并运行验证。' },
  { icon: '≡', title: '完善项目文档', prompt: '分析当前项目结构，完善 README 的安装、运行和功能说明。' },
];
...
export default function Home() {
  const [task, setTask] = useState('修复示例项目中失败的测试，只做必要修改，并确保所有测试通过。');
  ...
  useEffect(() => {
    fetch(`${API_BASE}/api/health`)
      ...
      .then((data) => setBackendMode(data.mode === 'model' ? 'model' : 'demo'))
  ...
  function attachStream(id: string) {
    ...
    stream.onmessage = (message) => {
      ...
      if (event.type === 'run_finished') {
        ...
        setRunStatus(nextStatus === 'completed' ? 'completed' : nextStatus === 'cancelled' ? 'cancelled' : 'failed');
        stream.close();
      }
    };
    stream.onerror = () => {
  ...
  async function startRun() {
    if (!task.trim() || runStatus === 'running') return;
    setNotice('');
    ...
    try {
      const response = await fetch(`${API_BASE}/api/runs`, {
        ...
        body: JSON.stringify({ task: task.trim(), workspace: workspace.trim() || '.' }),
      });
      ...
      <aside className="sidebar">
        ...
        <nav className="sidebar-nav" aria-label="任务导航">
          ...
          <button className="nav-item active" type="button">
            ...
            <span><strong>当前任务</strong><small>{submittedTask || '等待新的编程任务'}</small></span>
          </button>
          <button className="nav-item" type="button" onClick={resetDemo}>
            <span className="nav-icon">↻</span>
            <span><strong>重置演示项目</strong><small>恢复 calculator 故障</small></span>
          </button>

          <p className="history-label">最近任务</p>
          <div className="history-item"><span>修复计算器测试</span><time>今天</time></div>
          <div className="history-item muted"><span>为核心模块补测试</span><time>示例</time></div>
        </nav>

        <div className="sidebar-footer">
          <div className="connection-line">
            <span className={`connection-dot ${backendMode}`} />
            <span>{backendMode === 'model' ? '模型 API 已连接' : backendMode === 'demo' ? '本地演示模式' : '后端未连接'}</span>
          </div>
        ...
        <div className="conversation-scroll" ref={conversationRef}>
          {!submittedTask ? (
            <section className="welcome-state">
              <div className="welcome-mark"><span>›_</span></div>
              <p className="welcome-kicker">LOCAL CODING AGENT</p>
              <h1>今天想改什么？</h1>
              <p className="welcome-copy">描述一个编程任务。TraceCoder 会读取项目、修改代码、执行测试，并把每一步展示给你。</p>
              <div className="suggestion-grid">
                {SUGGESTIONS.map((suggestion) => (
                  <button type="button" key={suggestion.title} onClick={() => setTask(suggestion.prompt)}>
        ...
        <div className="composer-zone">
          ...
          <div className="composer-card">
            <textarea
              ...
              placeholder="描述任务，或让 Agent 修复一个问题…"
            />
            <div className="composer-toolbar">
              <label className="path-pill">
                <span>⌁</span>
